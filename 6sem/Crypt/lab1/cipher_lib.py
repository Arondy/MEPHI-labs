...
class CoolCipher:
    ...
    def __block_cipher_encrypt(self, plain_data_block: bytes) -> bytes:
        cipher = self.aes.encrypt(plain_data_block)
        return cipher

    def __ecb_encrypt(self, plain_data: bytes) -> bytes:
        cipher = self.__block_cipher_encrypt(plain_data)
        return cipher

    def __cbc_encrypt(self, plain_data: bytes) -> bytes:
        plain_data = self._xor_bytes(plain_data, self.chained_vector)
        cipher = self.__block_cipher_encrypt(plain_data)
        self.chained_vector = cipher
        return cipher

    def __cfb_encrypt(self, plain_data: bytes) -> bytes:
        cipher = self.__block_cipher_encrypt(self.chained_vector)
        cipher = self._xor_bytes(cipher, plain_data)
        self.chained_vector = cipher
        return cipher
    ...
    def __cfb_decrypt(self, encrypted_data: bytes) -> bytes:
        plain_data = self.__block_cipher_encrypt(self.chained_vector)
        plain_data = self._xor_bytes(plain_data, encrypted_data)
        self.chained_vector = encrypted_data
        return plain_data
